...
\begin{proof}[Beweis des Satzes]
	...
	$$\bar{f}(abN) = f(ab) = f(a)f(b) = \bar{f}(aN)\bar{f}(bN)$$ für alle $a, b \in G$.
        ...
        Angenommen es existiere eine weitere Abbildung $\tilde{f}$ mit obigen Eigenschaften. So gilt für $aN \in G/N$
        \[ \tilde{f}(aN) = \tilde{f}(\pi(a)) = f(a) = \bar{f}(\pi(a)) = \bar{f}(aN) \]
        aufgrund der Surjektivität von $\pi$. Daraus folgt $\tilde{f} = \bar{f}$.
\end{proof}
...
\begin{proof}
	...
	\item Es sei $\sigma\in S_n, \sigma\neq\text{id}$. Für $m_1,m_2\in\{1,\dots,n\}=:M$ definieren wir 
		...
		Dann ist $\sim$ eine Äquivalenzrelation auf $M$. Es sei $M = K_1\cup K_2\cup\dots\cup K_r$ die Zerlegung von $M$ in disjunkte Äquivalenzklassen.
                Wir betrachten $a_j\in K_j$ beliebig. Es sei $l_j$ die kleinste positive Zahl mit $\sigma^{l_j}(a_j) = a_j$.\footnote{Ein solches $l_j$ existiert aufgrund der vorausgesetzten Endlichkeit der symmetrischen Gruppe nach dem Satz von Lagrange. Es gilt also $l_j \leq \ord(\sigma)$.} % FIXME Existenz? -- Fixed?
	...
	\item Es sei $\sigma\in S_n$ und $(m_1\dots m_k)$ ein Zyklus. Dann gilt für $\alpha = 1,\dots,k$ 
	...
	\item Wegen (a) genügt es zu zeigen, dass für alle $k\in\mathbb{N}$ alle k-Zyklen Produkte von Transpositionen sind.
		...
		2 & 3 & 4 & 1
		\end{pmatrix}.\]
		Anmerkung: Es reichen deutlich weniger Transpositionen aus, um $S_n$ zu erzeugen\footnote{So gilt für $n \geq 2$ bereits $S_n = \langle (1 2) , (1 \dots n) \rangle$. Siehe Übung 3, Aufgabe G3.}.\qedhere
	\end{enumerate}
\end{proof}
...
\begin{ex}
	...
	\item $G$ operiert auf sich selbst durch Konjugation.
		...
		ist ein Gruppenhomomorphismus, d.h. eine Operation von $G$ auf sich selbst.
		\begin{proof}
...
Wenn G eine echte Untergruppe H enthält, wobei $|H|$ durch p teilbar ist, existiert nach der Induktionsvoraussetzung eine Untergruppe von H von der Ordnung $p^\alpha$.(fertig) 
%FIXME Damit die im Beweis vorgenommene Fallunterscheidung gilt, sollte angenommen werden, dass |H| durch p^n teilbar ist!

Wir können uns auf den Fall beschränken, dass $p^n \nmid \ord(H)$ für alle Untergruppen $H\neq G$ gilt. 
Sei $(a_{i}),{i \in I}$ ein Repräsentantensystem der (nicht-zentralen) Konjugationsklassen, so gilt
\[ p^n q=|G|=|Z(G)|+\sum_{a_{i}\notin Z(G)}\dfrac{|G|}{|C_{G}(a_{i})|}. \]
$p^n \nmid |C_{G}(a_{i})|$ ergibt sich aus der Annahme $p^n \mid |G|$.  %FIXME: Folgt eher aus der Annahme, p^n \nmid \ord(H) f.a. Untergruppen (Fabian)
Daraus folgt: $\dfrac{|G|}{|C_{G}(a_{i})|}$ ist durch p teilbar. 
%Verbesserungsvorschlag:
%Da nach Voraussetzung $p^n \nmid |C_{G}(a_{i})|$, ergibt sich mit der Annahme $p^n \divides |G|$, dass $\dfrac{|G|}{|C_{G}(a_{i})|}$ für alle $i \in I$ durch p teilbar ist.
Daraus folgt, dass $|Z(G)|$ durch p teilbar ist. 
...
Da $b$ im Zentrum von $G$ liegt, ist $\langle b \rangle$ eine normale Untergruppe von $G$. Wir betrachten die Faktorgruppe $G/ \langle b \rangle$. Es gilt
	...
	\item Sei $H$ eine $p$-Untergruppe und $S$ eine $p$-Sylowuntergruppe. $H$ operiert auf der Menge $X = G/S$ durch
	$$ H \times X \rightarrow X, h \cdot gS := (hg)S.$$
	Es gilt $|X| = \frac{|G|}{|S|} = \frac{p^nq}{p^n} = q$. Da $H$ eine $p$-Gruppe ist, gilt $|X^H| = |X| = q \ (\mod p)$. Da $q$ prim zu $p$ ist, ist $|X^H|$ nicht durch $p$ teilbar. Insbesondere kann $X^H$ nicht leer sein, denn $p \divides 0$. Es gibt also ein $g \in G$ mit $gS \in X^H$. Für alle $h \in H$ gilt dann $(hg)S = gS$ und somit $g^{-1}hg \in S$ und $h \in gSg^{-1}$. Dies liefert, dass $H$ in $gSg^{-1}$ enthalten ist.
...
Bevor wir den dritten Teil des Sylow-Satzes beweisen, wenden wir uns folgender allgemeiner Frage zu: Wie bestimmt man die Anzahl der zu einer gegebenen Untergruppe konjugierten Untergruppen? Es sei $\mathcal{U}$ die Menge aller Untergruppen von G und $ G \times \mathcal{U} \rightarrow \mathcal{U}, (g , H) \mapsto g \cdot H := gHg^{-1} $ eine Gruppenoperation.
